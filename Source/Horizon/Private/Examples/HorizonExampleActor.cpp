...
void AHorizonExampleActor::SendJSONMessage(const FString& Namespace, const FString& Event, const TMap<FString, FString>& Data)
{
	...
	FString JSONMessage = UHorizonUtility::CreateJSONMessage(Namespace, Event, MessageData);
...
void AHorizonExampleActor::SendBinaryTestMessage()
{
	...
	TArray<uint8> BinaryData = UHorizonUtility::StringToByteArray(TestString);
...
void AHorizonExampleActor::OnWebSocketConnected(bool bSuccess)
{
	if (bSuccess)
	{
		...
		SendJSONMessage(TEXT("core"), TEXT("player_connected"), ConnectionData);
...
void AHorizonExampleActor::OnWebSocketMessage(const FString& Message)
{
	...
	{
		UE_LOG(LogHorizon, Log, TEXT("Parsed JSON Message: Namespace=%s, Event=%s, Data=%s"), *Namespace, *Event, *Data);
	}
}
...
void AHorizonExampleActor::OnWebSocketRawMessage(const TArray<uint8>& Data, int32 Size, int32 BytesRemaining)
{
	...
	if (bLogReceivedMessages)
	{
		FString DataAsString = UHorizonUtility::ByteArrayToString(Data);
...
void AHorizonExampleActor::SendPeriodicMessage()
{
	...
	SendJSONMessage(TEXT("status"), TEXT("update"), StatusData);
}
